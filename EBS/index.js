...
EBSData.prototype.merge = function (obj) {
    for (var attrname in obj) {
      if (typeof obj[attrname] !== 'function') this[attrname] = obj[attrname];
    }
  }
  /**
...
function Instrument(payment) {
  if (payment.payment_mode) this.payment_mode = payment.payment_mode;
  if (payment.payment_option) this.payment_option = payment.payment_option;
  if (payment.bank_code) this.bank_code = payment.bank_code;
  if (payment.emi) this.emil = payment.emi;
  if (payment.card_brand) this.card_brand = payment.card_brand;
  if (payment.name_on_card) this.name_on_card = payment.name_on_card;
  if (payment.card_number) this.card_number = payment.card_number;
  if (payment.card_cvv) this.card_cvv = payment.card_cvv;
  if (payment.card_expiry) this.card_expiry = payment.card_expiry;
}
/**
...
function Transaction(transaction) {
  ...
  if (transaction.ba_bankaccountcode[0] != '' && transaction.split_amount[0] != '') {
    ...
    for (i in transaction.ba_bankaccountcode) this[transaction.ba_bankaccountcode[i]] = transaction.split_amount[i];
  }
}
/**
...
Merchant.prototype.signTransaction = function (user, transaction, instrument) {
  ...
  if (!user && !transaction) return "User and Transaction Required";
  else {
    data.merge(user);
  ...
  for (i in config.HASHSEQUENCE) {
    ...
    if (typeof data[key] !== 'undefined' && data[key] !== null && data[key] !== '') {
      if (key == 'ba_account_code') {
        for (i in data[key]) preHash += '|' + data[data[key][i]];
        delete data['ba_account_code'];
      } else preHash += '|' + data[key];
    }
  }
...
Merchant.prototype.initiatePayment = function (signedTxn, callback) {
  var error;
  if (!signedTxn && !signedTxn.secure_hash) return callback("Merchant signed data required.");
  return callback(error, pug.renderFile(formTemplate, {
    data: signedTxn
  }))
}
module.exports = {
